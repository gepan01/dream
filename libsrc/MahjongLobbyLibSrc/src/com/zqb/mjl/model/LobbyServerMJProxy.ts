module MJLobby {
    export class LobbyServerMJProxy extends Proxy {
        ...
        public initServer(): void {
            ...
            if (!NetMgr.ws || !NetMgr.ws.isConnected) {
                this._timeOutId = egret.setTimeout(() => {
                    MJLobby.LobbyEvents.Instance.dispatchEvent(new egret.Event(LobbyEvents.NOTIFY_CONNECT_TIMEOUT));
                }, this, 10000);
        ...
        public loginOut(): void {
            NetMgr.logout();
            MJLobbyData.getInstance().myBaseInfo=null;
        }
		/**
         * http平台登录完成
         */
        private onHttpInitSucc(obj: any): void {
            ...
            if (MJLobbyData.getInstance().myBaseInfo) {
        ...
        private onHttpInitFail(rev:any): boolean {
            if(rev && rev.retcode){
                if(rev.retcode==2){
            ...
            if (this.httpInitFailTimes == 0) {
                ...
                NetMgr.init("http://211.159.149.160:7000/httplogin", this.gameId, this.zoneId, this.onHttpInitSucc, this.onHttpInitFail, this);
                uniLib.Console.log("尝试第1次登陆重连IP:118.89.55.208");
                return true;
            } else if (this.httpInitFailTimes == 1) {
                this.httpInitFailTimes++;
                NetMgr.init("http://118.89.55.208:7000/httplogin", this.gameId, this.zoneId, this.onHttpInitSucc, this.onHttpInitFail, this);
        ...
        private onSockInitSucc(): void {
            ...
            if (uniLib.Global.isInGame == false) {
                ...
                uniLib.CommonModelMgr.Instance.registerCommonModel(uniLib.CommonModel.LOBBY_NOTICE, GlobalControl);
                uniLib.Global.addEventListener(uniLib.ZqEvent.EVENT_G2L, this.onGameGoon, this);
                this.onLoginServer();
            }
        }
        private onGameGoon(e: uniLib.ZqEvent) {
            if (e.param == "game_request_continue") {
                var req: Cmd.ApplyContinuePlayRoomCmd_C = new Cmd.ApplyContinuePlayRoomCmd_C();
                this.sendData(req);
            }
        }
        ...
        private onConnectFail(): boolean {
            if (uniLib.Global.isH5) {
                uniLib.UIMgr.instance.hideLoading();
                LobbyPopupManager.showConfirmPanel("请检查网络状况", [""], [this.refresh], "", this);
            } else {
        ...
        private onLoginServer() {
            var login: Cmd.UserInfoSynLobbyCmd_C = new Cmd.UserInfoSynLobbyCmd_C();
            login.lobbyId = MJLobbyData.getInstance().lobbyId;
